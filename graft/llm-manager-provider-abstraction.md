---
name: LLM Manager & Provider Abstraction
slug: llm-manager-provider-abstraction
type: system
sources:
...
sources_digest: 61c41938962a92bb8e775d72c66758f37879ee3ab89253a44b13b1a7e7e03463
links:
  - to: autoclip-auxiliary-services
    relation: uses
    description: >-
      seedance_prompt_generator, publish_material_generator, and
      script_optimizer all reuse get_llm_manager for their model calls.
  - to: autoclip-fastapi-service
    relation: uses
    description: >-
      main.py uses get_llm_manager for model configuration and integrates the
      services.
  - to: autoclip-pipeline-stages
    relation: uses
    description: >-
      All four pipeline steps call LLMClient/LLMManager for outline, timeline,
      scoring, and title generation; shared_config supplies prompt file paths
      and API keys.
generator:
  version: 1
covers:
...
The unified multi-provider LLM layer: LLMManager loads persistent settings from a JSON file (with env-var fallbacks), delegates to LLMProviderFactory/LLMProvider implementations for DashScope, OpenAI, Gemini, SiliconFlow, and Ollama (local vision). Separates persistent settings from runtime model overrides so pipeline-specific selections never corrupt user config. call_with_retry does exponential backoff (3 attempts) but deliberately skips retries on ValueError. OllamaClient is a separate stdlib-only path for local frame analysis that silently degrades to None on failure so the main pipeline never blocks on it.

## Related

- uses [[autoclip-auxiliary-services]] — seedance_prompt_generator, publish_material_generator, and script_optimizer all reuse get_llm_manager for their model calls.
- uses [[autoclip-fastapi-service]] — main.py uses get_llm_manager for model configuration and integrates the services.
- uses [[autoclip-pipeline-stages]] — All four pipeline steps call LLMClient/LLMManager for outline, timeline, scoring, and title generation; shared_config supplies prompt file paths and API keys.
