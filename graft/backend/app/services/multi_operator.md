...
- sync_profiles_from_db · function · L322-L371 — Loads enabled PublishProfiles from DB, allocates/reuses a port per profile, registers routes, and writes the pub:profiles list so ports persist across restarts.
- save_pending · function · L377-L383 — Persists a structured publish payload (without page objects) to Redis with a 30min TTL for idempotent retry.
- get_pending · function · L386-L392 — Reads a pending publish payload from Redis.
- delete_pending · function · L395-L400 — Removes a pending publish payload from Redis.
- freeze_pending · function · L403-L418 — Freezes a pending payload on selector mismatch so scheduling neither retries nor switches operators, preventing accidental sends or partial fills.
- issue_cdp_token · function · L424-L439 — Issues a short-lived single-scope cdp_proxy access token bound to actor+account to prevent replay.
- verify_cdp_token · function · L442-L460 — Validates a cdp_proxy token for existence, expiry, and account match, then consumes it (deletes) so it can only be used once.
- _fmt_ts · function · L466-L473 — Converts a unix-second string from the route table into a readable ISO timestamp.
- get_route_matrix · function · L476-L513 — Builds the operator port matrix dashboard by reading all routes and aggregating per-operator daily quota consumption.
- get_verification_status · function · L516-L586 — Produces a real-time verification-wizard status report aggregating flag, profiles, routes, quotas, pending, risk/login audit, and cdp token counts for step-by-step acceptance.
- get_operator_stats · function · L589-L612 — Aggregates per-operator quota consumption and inflight counts for the dashboard.
