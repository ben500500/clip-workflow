...
- get_presigned_url · function · L202-L242 — Generates a browser-accessible presigned GET URL, preferring the external endpoint and optionally forcing attachment download via response headers.
- get_presigned_upload_url · function · L245-L265 — Generates a presigned PUT URL using the internal endpoint for container-side Worker uploads.
- delete_file · function · L268-L278 — Deletes an object from MinIO asynchronously, returning success status.
- _list_objects_sync · function · L281-L293 — Synchronous helper that lists objects under a prefix and collects their key, size, etag, and last-modified metadata.
- list_files · function · L296-L304 — Lists files in a bucket under a prefix asynchronously, returning an empty list on S3 errors.
- get_file_info · function · L307-L321 — Fetches metadata for a single object asynchronously, returning None if the object does not exist.
- ensure_bucket · function · L324-L336 — Checks whether a bucket exists and creates it if missing, returning success status.
- download_to_file · function · L339-L362 — Downloads a MinIO object to a local file, creating parent directories with explicit mode to avoid executable-only dirs.
