...
- getClipTypeLabel · function · L53-L61 — function getClipTypeLabel(clipType: string | null | undefined): string
- getClipTypeColor · function · L63-L70 — function getClipTypeColor(clipType: string | null | undefined): string
- getStatusColor · function · L72-L98 — Maps a workflow status key to a hex color for UI badges, defaulting to gray for unknown statuses.
- getStatusLabel · function · L100-L126 — Maps a workflow status key to its Chinese display label, falling back to the raw key for unknown statuses.
- truncateText · function · L128-L131 — Truncates long text to a maximum length with an ellipsis suffix, returning '-' for empty input.
