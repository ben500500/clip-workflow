...
| 方法 | 路径 | 说明 |
|------|------|------|
| `POST` | `/api/shortdrama/prompt/generate` | 根据文案一次生成提示词三版本（长提示词/短提示词固定模板 + AI提示词，复用 AutoClip 模型，支持 10s/15s/20s/25s/30s/自定义时长，所选时长即保存为当前用户默认值） |
| `POST` | `/api/shortdrama/prompt/optimize` | 短剧文案 AI 优化：调用模型改写文案（增强冲突/悬念/情绪张力，保留主线与反转，保持对白旁白格式，人名地名用代称） |
| `GET` | `/api/shortdrama/prompt/default-duration` | 获取当前登录用户的提示词生成默认时长（未设置返回 15s） |
| `PUT` | `/api/shortdrama/prompt/default-duration` | 保存当前登录用户的提示词生成默认时长 |
...
| `/watermark` | 短片制作 | 提示词生成三版本（AI提示词/长提示词/短提示词，长/短模板可在线编辑持久化，复用 AutoClip 模型，10s/15s/20s/25s/30s/自定义时长，所选时长即作为当前登录用户默认值，题材/基调/角色下拉预设，短剧文案输入框支持 AI 优化） + 成片视频上传/一键导入去水印 + 四套开源去水印引擎切换（seedance_wm / remove_mask / RAiW / Seedance 2.0）、批量上传/下载、异步进度、任务历史 + 短剧发布素材生成（短标题/三款配文/成套标签/三条神评），提示词→去水印→发布任务 id 关联自动代入文案，v4/v5 去水印；v6 提示词生成；v6.1 合规代称/自定义时长/预设下拉/历史上传视频导入去水印；v7 发布素材生成；v8 remove_mask ROI 经验库引擎；v9 提示词三版本 tab；v10 模板可编辑+Tab 顺序+历史去时间+状态重置+任务关联；v11 时长预设扩展 20s/25s/30s+用户默认时长；v12 文案 AI 优化+历史操作列固定可见+去水印任务名称日期+4位自增；v13 同步 remove-mask 上游 v7 自动水印分析（任意视频先分析再处理 + 爷孙重逢预置 ROI）；v14 同步 remove-mask 上游 v10 四角检测/时间一致性过滤/文字带高度守卫/中文文件名匹配 + 爷孙重逢 TR+BR ROI 修正）） |
...
| 日期 | 变更 |
|------|------|
| 2026-08-12 | 节点引擎推送更新：服务器端修改引擎脚本（slice.py 等）后可在 Worker 节点管理页点「推送更新」，后端打包最新 engines/ 下发指令，节点自动拉取并替换本地引擎，无需重新部署/重启；节点心跳上报引擎版本，界面展示本地 vs 服务器版本（不一致标黄提示） |
| 2026-08-12 | Ubuntu 节点部署包：`slice-worker/ubuntu/` 提供预编译二进制 + 引擎 + 一键部署脚本（systemd 托管/自动装依赖/开机自启），`build_package.sh` 生成分发包，适配 Ubuntu 20.04~24.04（x86_64/arm64） |
| 2026-08-12 | 一键切片接入 AI 智能选点 + 通用区间检测：配置项新增 autoclip（开关/候选数/评分/时长/画面理解）与 interval（开关/检测模式），按序执行「上传→AI选点→自动审核→区间检测→切片→删源」 |
...
| 2026-08-12 | 横屏切片固定文字角标：在成品指定位置叠加固定文字（最左侧竖排/左下角/右上角等，无需上传图片，支持自定义内容/字号/字体色/描边色/竖排），前端、后端、Worker/Celery、引擎全链路透传 |
| 2026-08-12 | 横屏切片默认字幕按样图定标：默认字号比例提高（约占画面 7%），字幕位置微调，默认字幕更清晰 |
