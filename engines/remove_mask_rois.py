...
def match_rois(source_name: str) -> Optional[dict]:
    """按原始文件名匹配内置 ROI 经验库。
    ...
    if norm in VIDEO_ROIS:
        return VIDEO_ROIS[norm]
    for code in re.findall(r'[0-9A-Z]{8}', norm):
        if code in VIDEO_ROIS:
            return VIDEO_ROIS[code]
    return None


def resolve_rois(source_name: str, manual_region=None) -> dict:
