...
VIDEO_ROIS_SMALL = {
    ...
    # 爷孙重逢：水印为固定白色文字，位于右上（TR）与右下（BR），且位置随时间有小幅分段。
    #   实测（白色像素时间一致性 + top-hat 文字特征 + 跨帧位置固定性三重验证）：
    #   - 右上 TR：y≈140-160 与 y≈185-190，x≈640-693（早期+后期分段出现，位置固定）
    #   - 右下 BR：y≈1216-1254，x≈610-689（全程主要水印，Seedance 风格文字）
    #   注意：画面中上部 y≈120-167 x≈208-237 是老人头部/白发（移动主体，绝不可覆盖）；
    #   左上 TL（y≈32-66 x≈48-107 等）经位置追踪确认为中段短暂画面内容，非固定水印，不覆盖。
    #   相比旧版：不再覆盖老人头部区域（此前 TL(25,145,20,245) 误伤白发），
    #   并把 TR 移到真实水印位置（旧 TR(25,135,650,720) 把 y145-180 的水印漏掉了）。
    '爷孙重逢': {
        # v11（同步 remove-mask 上游）：修复左上角 TL 水印漏覆盖。实测（白色像素时间
        # 一致性 + top-hat 文字特征 + 跨帧位置固定性三重验证）：
        #   - 左上 TL：y≈0-66 x≈21-179，早期 x≈21-106 与后期 x≈46-179 两段 Seedance 文字
        #   - 右上 TR：y≈140-160 与 y≈185-190，x≈640-693（早期+后期分段出现）
        #   - 右下 BR：y≈1216-1254，x≈610-689（全程主要水印）
        # TL 覆盖范围（y0-72 x20-184）严格限制在左上角，不与老人头部（y90+ x190+）重叠。
        'TL': (0, 74, 18, 186),        # 覆盖左上角 Seedance 水印 logo（含两段文字，+4px buffer）
        'TR': (136, 194, 636, 698),    # 覆盖右上两段水印 y140-190 x640-693（+2px buffer）
        'BR': (1208, 1260, 604, 694),  # 覆盖右下水印 y1216-1254 x610-689（+4px buffer）
    },
}
