...
> 版本：v1.6（含 PR #187 风控分级识别 + PR #211 衔接修复 + 阶段⑩人在环兜底）| 日期：2026-08-19
...
> ⚠️ **v1.3 修订（对应端到端实测报告问题修复）**：① **发布假成功闭环**（P0-1）——发布结果现以**成功页 URL 命中**为主判据，超时/未确认会正确置 `failed`（不再是假成功）；上传未完成即失败（P1-3）；标题超 16 字自动截断（P1-4）；② **扫码会话存活**（P0-2）——扫码后登录页标签页保持打开，微信扫码后 Web 会话不再被 `page.close()` 杀掉；③ **选点参数 falsy 修复**（P1-5）——`min_score`/`min_duration` 显式传 `0` 表示「不限」，不再被 `or 60`/`or 0` 回退吞掉。
> ⚠️ **v1.4 修订（发布链路最后三公里）**：① **视频号话题拼进描述**——视频号**没有独立话题框**，`tags` 以 `#话题#` 形式自动拼进描述末尾（不再找独立标签输入框），Agent 无需单独填标签；② **一键生成发布素材**——新增 `POST /api/shortdrama/publish-material/generate-from-output`，从切片成品自动组装剧情梗概并生成短标题/三款配文/成套标签/三条神评；③ **发布后置顶神评**——发布成功后可自动发表并置顶互动神评（探活式，失败不阻断发布）；④ **三款配文选一款**——发布弹窗支持在悬念钩子/精简爆款/情绪爽文间切换配文。
> ⚠️ **v1.5 修订（PR #187 风控分级识别，已合并 main）**：① 发布链路新增**上传/环境级风控分级**——`UploadRiskError` + `_probe_upload_risk_signal()` 从页面文本/URL/弹层探测 `env_risk`/`upload_limited`/`need_login`，风控拒发与普通超时**不再混为一谈**；② 失败任务的 `risk_type` 细分：默认 `publish_limited`，风控拒发落 `upload_limited`/`env_risk`；③ **上传前预检**——命中 `need_login`/风控会提前失败，不进入上传；④ **风控拒发不自动重试**（避免重复消耗账号安全额度），走死信队列人工/定时重放。
> ⚠️ **v1.6 修订（PR #211 衔接修复 + 阶段⑩人在环兜底，已合并 main）**：① 批量切片阶段衔接 3 处竞态修复（`_wait_slice` 排除 `detect_%` 复用表记录 + 按 `slice_task_id` 精确轮询、`_wait_detect` 按 `detect_task_id` 轮询、`dispatch_ready_slices` 先落 `slice_task_id` 再置 `phase=slicing`）；② 新增阶段⑩「人在环兜底」——无真人化 worker 且被环境级风控拒发时，制备齐全后由人用真实浏览器 + OS 级文件对话框完成最后发布并回填 `published_url` 闭环。
...
> ⚠️ **P1-5（v1.3）选点参数 falsy 修复**：
> - `min_score_threshold`：**显式传 `0` 表示「最低分不限」**（不再被 `or 60` 回退吞掉）；传 `null`/不传时用默认 `60`。
> - `min_duration` / `max_duration`：**显式传 `0` 表示「时长不限」**，会显式传给引擎（不再被 `and x>0` 跳过）；传 `null`/不传时 `min_duration` 默认 `30`、`max_duration` 默认 `0`（不限上限）。
> - 若你的素材多为**短视频切片（<30s）**，建议把 `min_duration` 显式设 `0`（或调低到几秒），否则系统默认 30s 会把短片段过滤掉，导致选点无产出。
...
> ⚠️ **P1-4（v1.3）**：视频号**短标题上限 16 字**，超限会触发页面红字且「发表」按钮置灰（假成功诱因）。系统已在填充前**自动截断到 16 字**，Agent 无需自行截断，但建议源端就控制标题 ≤16 字，避免截断丢失关键文案。
> ⚠️ **v1.4 话题拼进描述**：视频号**没有独立话题框**，`tags` 数组会被系统以 `#话题#` 形式**自动拼进描述末尾**（`_merge_tags_into_description`），Agent **无需**单独去填独立话题输入框；抖音/快手有独立话题框，仍按 `tags` 独立写入。
...
- [ ] **若 `status == "failed"`（P0-1 修复后不再假成功）**：说明发布结果未被确认（成功页 URL 未命中/超时/上传未完成），任务已置失败并写**死信**（`dead_letter=true`，`error_message` 带原因）。Agent 应：
    - 记录失败原因（`error_message`）与 **`risk_type`**（v1.5/PR #187 新增分级）：
        - `upload_limited` / `env_risk` → **上传/环境级风控拒发**（如 `300001`/`upload_params`）——**不要自动重试**（重复消耗账号安全额度），记录后应报告人工/定时重放，等待环境冷却；
        - `need_login` → 登录态失效，走重新扫码登录（阶段 A 第 3 步）后再发；
        - 默认 `publish_limited` → 普通发布受限/失败，可评估瞬态原因后重发。
    - 若是可重试的瞬态原因（非风控），可重新创建发布任务或对死信任务走重发接口；
    - **不要**误判为成功/继续等待——`published_url` 为空即代表未发布。
...
### ⑩ 人在环兜底（无「真人化 worker」时的安全退路，v1.6 新增）

> **适用场景**：当前部署环境**无可用「真人化 worker」**（本机已登录真实 Edge/Chrome + OS 级文件对话框），
> 且自动发布被**微信环境级风控**拦截（`env_risk`/`upload_limited`，典型 `300001`/`upload_params`，
> 数据中心 IP + Playwright Chromium + `set_input_files` 浏览器内部注入触发）。
> 走此兜底：系统先把链路**制备齐全**，产出「一键真人发布」操作清单，由人用**真实浏览器 + OS 级文件对话框**完成最后发布。

**Step 1 · 判定兜底**：发布任务 `risk_type == "env_risk"`/`"upload_limited"` 且已写死信
（`dead_letter=true`）→ **不要自动重试**（重复消耗账号安全额度）。

**Step 2 · 制备齐全（自动化，不涉真实浏览器）**：
- 确认素材/选点/切片/发布素材（标题/三段配文/标签/三条神评）均已落库，`GET /publish/tasks/{id}`
  可查到完整发布 payload；
- 把最终成片路径、标题、描述、话题、神评、目标账号（含所属运营者）整理成「一键真人发布」清单；
- 标记 `disposition = "manual_publish"`，等待人工/定时重放。

**Step 3 · 一键真人发布（Computer Use / 真人完成最后一步）**：
1. 用**运营者本人微信已登录的真实浏览器**（本机 Edge/Chrome，真实/家庭 IP）打开视频号创作中心发布页；
2. 选视频走 **OS 级文件选择对话框**（系统 Finder/资源管理器，**不要用 `set_input_files` 浏览器注入**）；
3. 用清单里的文案填标题/描述/话题，选封面、挂小程序链接；
4. 点发布，成功后把 `published_url` 回填发布任务（或运维手动确认），链路闭环；
5. **风控冷却**：连续失败遵守 2–24h 冷却窗口；同 IP 小时 ≤2 条。
...
> 🆕 **v1.3 新增修复**（对应《端到端产线实测报告》）：以下 5 项已在本次 PR 编码落地——

> 🆕 **v1.4 新增能力**（发布链路最后三公里）：
> - **视频号话题拼进描述**：视频号无独立话题框，`tags` 以 `#话题#` 拼进描述（`_merge_tags_into_description`），避免标签静默丢失。
> - **一键生成发布素材**：`POST /api/shortdrama/publish-material/generate-from-output`，从切片成品组装剧情梗概→生成短标题/三款配文/成套标签/三条神评（短片制作链路复用）。
> - **发布后置顶神评**：发布成功后可自动发表+置顶互动神评（探活式，失败不阻断发布）。
> - **三款配文选一款**：发布弹窗支持悬念钩子/精简爆款/情绪爽文切换配文。
### ✅ P0-1（实测）：发布「假成功」——超时/未确认仍置 `published`

**状态**：✅ **已修复**（发布结果以**成功页 URL 命中**为主判据，超时抛 `PublishTimeoutError` 置 `failed`，不再假成功）

**问题描述**：`_wait_for_publish` 超时用 `except: return (url, None)` 兜底，且 `publish()` 无条件置 `published`；`[data-id]` 选择器过宽易命中无关元素，导致没发出去却报成功。

**位置**：`backend/app/services/publish_service.py` → `_wait_for_publish` / `confirm_publish`

### ✅ P1-3（实测）：上传完成检测失效——上传区为空仍继续

**状态**：✅ **已修复**（`_wait_for_upload` 改为等待真实 `<video src>` 预览元素，未就绪即抛异常走失败）

**位置**：`backend/app/services/publish_service.py` → `_wait_for_upload`

### ✅ P1-4（实测）：标题超 16 字限制——「发表」按钮置灰/红字

**状态**：✅ **已修复**（`_set_title` 填充前自动截断 ≤16 字）

**位置**：`backend/app/services/publish_service.py` → `_set_title`

### ✅ P0-2（实测）：扫码会话「必死」——`page.close()` 杀登录会话

**状态**：✅ **已修复**（`capture_login_qr` 等不再 `page.close()` 登录页标签页，且优先复用已有 channels 页面；扫码后 Web 会话可正常建立）

**位置**：`backend/app/services/login_qr_service.py` → `capture_login_qr` / `check_login_status_via_cdp` / `silent_keepalive`

### ✅ P1-5（实测）：选点参数 falsy 陷阱——`min_score_threshold`/`min_duration` 传 0 被回退

**状态**：✅ **已修复**（`is not None` 判断替代 `or`，显式传 0 = 不限；`min_duration=0` 显式传给引擎）

**位置**：`backend/app/celery/tasks.py` / `backend/app/services/autoclip_service.py` → `get_clips`
