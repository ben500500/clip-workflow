...
13. **创建发布任务**：`POST /publish/tasks`（201）。
    ...
    - **视频号话题拼进描述（v1.4）**：视频号**没有独立话题框**，`tags` 数组会被系统以 `#话题#`
      形式**自动拼进描述末尾**（`_merge_tags_into_description`），Agent **无需**单独填独立话题输入框；
      抖音/快手有独立话题框，仍按 `tags` 独立写入。
    - **可选：一键生成发布素材（v1.4）**：可先调
      `POST /shortdrama/publish-material/generate-from-output`（入参 `{output_id}`），系统会从切片
      成品自动组装剧情梗概并生成 `short_title` / 三款 `captions` / 成套 `tags` / 三条 `comments`，
      再拿返回的标题/描述/标签填发布任务；`material_id` 关联后发布成功会自动发表+置顶神评。
...
## v1.4 相对 v1.3 的新增能力（发布链路最后三公里）

| # | 能力 | 说明 | 落点 |
|---|---|---|---|
| 1 | **视频号话题拼进描述** | 视频号无独立话题框，`tags` 以 `#话题#` 自动拼进描述末尾，Agent 无需填独立话题输入框 | `_merge_tags_into_description` / publish_service.py |
