...
# 真实短剧素材校准（de-du b22a8f0，batch 71f6f10d 反馈）：crop 后画面占比变化叠加降饱和
# 会使 Δsat 超阈（第4集 crop=0.19/0.20 时 sat=0.97/0.98 实测 Δsat -0.051~-0.058），
# 去掉 0.20 收敛到 15~19%，最低饱和收敛到 0.95 仍保留可感知差异（0.97/0.98 会非线性放大 Δsat）。
_CROP_POOL = [0.15, 0.16, 0.17, 0.18, 0.19]
_SPEED_POOL = [1.05, 1.06, 1.07, 1.08]
# 真实短剧素材校准（de-du b22a8f0）：crop 叠加降饱和会放大 Δsat，去除 0.93（非线性放大），
# 收敛到 0.95~1.0 微妙范围，既保留差异化又消除明显发灰。
_SATURATION_POOL = [0.95, 0.96, 1.0, 0.95]
...
_ROLLBAND_POOL = [0]
_JITTER_POOL = [0]
# 真实短剧素材校准（de-du 8acfa6b）：0.7/0.9 强锐化会放大边缘冷暖对比、观感偏硬，易被察觉，收紧到 [0.2,0.3,0.4]
_SHARPEN_POOL = [0.2, 0.3, 0.4]
_WATERMARK_POOL = [
    None,
...
_COLORBALANCE_POOL = [
    # 单一中性值：彻底关掉色罩（旧池 .02/.01 级别 rs/bs 偏移会造成明显冷/暖色罩）——对齐 de-du bca0058
    "rs=0:gs=0:bs=0:rm=0:gm=0:bm=0",
]
# 真实短剧素材校准（de-du 8acfa6b/b22a8f0）：temperature=6400（偏冷）与 crop 叠加实测 Δsat 超阈
# （第4集 sat=0.95+temp=6400 Δsat=-0.045），进一步收紧到 6480~6520K（±20K 几乎无感），彻底消除冷蓝/暖黄色罩。
_TEMP_POOL = ["temperature=6500", "temperature=6500", "temperature=6480", "temperature=6520"]
# hue 色相旋转（度）：标准 libavfilter hue 滤镜。真实素材上 ±0.5° 仍会与 crop 叠加产生可感色罩
# （第4集 temp=6400+hue=+0.5 实测 Δsat -0.051>0.05；第2集 sat=1.0+hue=-0.5 实测 Δsat +0.055>0.05），
# 彻底关闭 hue 旋转，由 crop/speed/noise/scanline 结构差异承担去重（de-du b22a8f0）。
# 注：_regenerate_recipe 的 hue 钳位仍保留 ±6° 上限（尊重用户手动传入的大值），仅默认随机池关闭。
_HUE_POOL = [0.0, 0.0, 0.0]
