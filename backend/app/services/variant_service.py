...
# 收紧：仅 ±2-5% 微妙范围，最低 0.93（避免旧池 0.72 造成的明显发灰）——对齐 de-du bca0058
_SATURATION_POOL = [0.95, 0.93, 0.97, 1.0, 0.96]
...
# 收紧色温到 ±100K，几乎无感（旧池 5800/5900/6100/6900 会产生明显冷暖色罩）——对齐 de-du bca0058
_TEMP_POOL = ["temperature=6500", "temperature=6400", "temperature=6600", "temperature=6500", "temperature=6550"]
# hue 色相旋转（度）：标准 libavfilter hue 滤镜。实测 ±2°/±3° Δsat 0.13~0.20(>0.05) 明显变色、
# 是肤色偏蓝/粉的根因；收紧到 ±0.5°（Δsat 0.041<0.05 不明显）保留微弱色相差异——对齐 de-du 406d366。
# 注：_regenerate_recipe 的 hue 钳位仍保留 ±6° 上限（尊重用户手动传入的大值），仅默认随机池收紧。
_HUE_POOL = [0.0, 0.0, 0.5, -0.5]
