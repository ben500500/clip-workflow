...
async def _bootstrap():
    ...
    """
    # 同一 pytest 进程里若有其它 sqlite 测试模块（例如
    # tests/integration/test_publish_dispatch_commit_race.py），它们也会改
    # app.database 的模块级 engine / session factory。故每次 bootstrap 都把
    # 它们重新指回**本模块**的测试引擎，使本模块用例与执行顺序无关。
    # （只装一次是不够的：谁后导入谁生效。见 PR #358 引入的模块间干扰。）
    dbmod.engine = _test_engine
    dbmod.async_session_factory = async_sessionmaker(
        _test_engine, class_=AsyncSession, expire_on_commit=False
    )
