...
const SLICE_MODE_HELP: Record<string, { label: string; desc: string }> = {
  ...
  dedupe: {
    label: '去重模式',
    desc: '切割时进行画面去重处理，采用「空间变换（缩放裁切）+ 时域变换（变速）+ 色彩变换（轻微降饱和）+ 轻微锐化/贴纸水印」组合；统一不做镜像，明显影响画质的噪点/扫描线/复古偏色/色温/暗角/抖动均去掉或降到最低，最大程度保留原画质。提供两套推荐配方（保守裁切降饱和=默认首选 / 复古扫描）及轻/标准/重三档，并支持「去重高级配置」逐项手动调整每个手段。适合批量发布到多个平台，降低查重风险。',
  },
  scrub: {
    label: '挖洞模式',
