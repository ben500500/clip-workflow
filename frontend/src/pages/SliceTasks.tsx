...
const SLICE_MODE_HELP: Record<string, { label: string; desc: string }> = {
  ...
  dedupe: {
    label: '去重模式',
    desc: '切割时进行画面去重处理，采用「空间变换（缩放裁切）+ 时域变换（变速）+ 色彩变换（降饱和/暖调）+ 质感（噪点/扫描线/暗角/锐化/贴纸水印）」组合；统一不做镜像。提供两套推荐配方（保守裁切降饱和=默认首选，画质优先 / 复古扫描=第二选择，老电视扫描线+噪点质感）及轻/标准/重三档，并支持「去重高级配置」逐项手动调整每个手段。适合批量发布到多个平台，降低查重风险。',
  },
  scrub: {
    label: '挖洞模式',
...
const SliceTasks: React.FC = () => {
  ...
  // 字幕高度（距画面底边比例，默认 1/4：字幕块中心约在 3/4 屏高处）
  ...
  return (
    <div>
      ...
      <Card size="small" style={{ marginBottom: 16 }}>
        <Space wrap>
          <Cascader
            value={mode === 'dedupe' ? ['dedupe', dedupePreset || 'std_crop_desat'] : [mode]}
            ...
            <Space direction="vertical" size={12} style={{ width: '100%' }}>
              ...
              <Space wrap align="center" size={8}>
                <Tooltip title="字幕高度：字幕块中心距画面底部的距离（占屏高比例）。默认 0.25 = 字幕位于画面下方 1/4 处；越小越贴底，0.5 ≈ 垂直居中">
                  <Text strong style={{ fontSize: 12, cursor: 'help' }}>字幕高度</Text>
                </Tooltip>
                <InputNumber
                  size="small"
                  min={0.02}
                  ...
                  onChange={(v) => setSubtitleMarginRatio(v ?? 0.25)}
                  style={{ width: 100 }}
                />
                <Text type="secondary" style={{ fontSize: 12 }}>默认 0.25（屏高 1/4 处）</Text>
              </Space>
              <Space wrap align="center" size={8}>
