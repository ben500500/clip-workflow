...
| 本次 | **修复「选点→切片」断链（40 生产）**：独立 AI 选点完成后不自动切片，用户手动建切片任务无候选上下文 → cutlist 空 → 整片切片（选点 3 个只出 1 个整片）。修法：`autoclip_task` 保存选点结果后调 `_autoclip_auto_dispatch_slice`，**复用前端「一键切片」`run_slice` 完整链路**（`SliceRunRequest(auto_accept_all=True)`：自动通过候选→生成 cutlist→建 SliceTask→按部署 `SLICE_ENGINE` 分发 worker/local）。门控：① 检测进行中的 `batch_slice_items`（批量有独立自动切片守护 `dispatch_ready_slices`）→ 跳过避免重复；② 无候选→跳过；③ 并发闸门/异常仅记日志不影响选点。`run_slice` 延迟 import（tasks→slice→remotion_tasks→tasks 潜在环规避）。**注意：163 SLICE_ENGINE=local 时自动切片会在 selection worker 内同步跑（测试机可接受）** | `backend/app/celery/tasks.py`（`_autoclip_auto_dispatch_slice` + `autoclip_task` 调用点） |
